function CollisionHandler(ctx, camera, mapArray){
    ...
    this.hasReachedGround = function (proto) {

        var y = Math.round((proto.position.y + proto.actorHeight) / TILE_SIZE) - 1;
        var x = Math.round((proto.position.x + proto.actorWidth) / TILE_SIZE) - 2;
    ...
    this.pushingAgainstWall = function (proto, direction) {

        var fromY = Math.round((proto.position.y) / TILE_SIZE) - 1;
        var x;
        if(direction == 'D')
            x = Math.round((proto.position.x + proto.actorWidth) / TILE_SIZE);
        else if(direction == 'A')
            x = Math.round(proto.position.x / TILE_SIZE);
        x = x - 3;
        var toY = Math.round((proto.position.y + proto.actorHeight) / TILE_SIZE) - 1;
